...
function intercept(console, kind) {
...
async function getElevation(lon, lat) {
    ...
    const xyz2url = (x, y, z) => "https://elevation3d.arcgis.com/arcgis/rest/services/WorldElevation3D/Terrain3D" +
...
// our map
//
var ml = maplibregl; // give it a shorter name
function divMarker(className, anchor = "center") {
...
GolfMap.holeZoom = 16;
GolfMap.courseZoom = 14;
GolfMap.selectCourseZoom = 10;
GolfMap.maxZoom = 19;
////////////////////////////////////////////////////////////
//
...
class ScoreCard {
    constructor() {
        ...
        this.golfHoleFeatures = GolfMap.the.getSource("golfHoleFeatures");
        ...
        const addHoleColumn = (table, holeNumber) => {
            // hole number cell
        ...
        const updateScore = (holeNumber, update) => {
            // advance tutorial
    ...
    updateTotals() {
        ...
        const fmtToPar = (toPar) => toPar == 0 ? "E" : toPar > 0 ? "+" + toPar : toPar < 0 ? String(toPar) : "";
...
class Path {
    constructor() {
        ...
        this.accuracyCircle = GolfMap.the.getSource("accuracyCircle");
        GolfMap.the.addLayer({
        ...
        this.pathLine = GolfMap.the.getSource("pathLine");
        GolfMap.the.addLayer({
            "id": "pathLineLayer",
            "type": "line",
            "source": "pathLine",
    ...
    async moveLocationMarker(pos, center) {
...
class Courses {
    ...
    async queryCourseFeatures(id) {
        ...
        const drivingRanges = features.filter((f) => f?.properties?.golf == "driving_range");
        const onDrivingRange = (f) => drivingRanges.some((range) => turf.booleanWithin(f.geometry, range));
        ...
        const courseFeatures = features.filter((f) => onCourse(f) && !onDrivingRange(f));
    ...
    async queryCourses(south, west, north, east) {
