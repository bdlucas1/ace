...
const btn = (name: string) => `<span class='main-button tutorial-icon ${name}'></span>`
const hand = "<span style='font-size: 120%'>👈</span>"
const link = (url: string, text: string) =>
    `<a href='${url}' target='_blank' onclick='event.stopPropagation()'>${text}</a>`
const action = (action: string) => `<span class='action ${action}'></span>`
